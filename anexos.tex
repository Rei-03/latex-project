...
\textbf{Programador asignado:} & Brayan Castellano Cruz \\
\hline
\textbf{Descripción:} & El sistema debía mostrar una lista de todos los circuitos con sus datos principales: código, carga actual (MW), estado (activo/avería/asegurado), bloque operativo. La vista debía poder actualizarse mediante un botón "Actualizar". \\
\hline
\textbf{Observaciones:} & Los circuitos asegurados se mostraban en verde, los averiados en rojo. Se incluyeron filtros por código y bloque. \\
\hline
\end{tabular}
\end{table}
...
\hline
\textbf{Nombre:} & Gestión de circuitos asegurados (críticos) \\
\hline
\textbf{Prioridad en negocio:} & Alta \\
\hline
\textbf{Puntos estimados:} & 1 \\
...
\textbf{Programador asignado:} & Brayan Castellano Cruz \\
\hline
\textbf{Descripción:} & El sistema debía permitir marcar/desmarcar circuitos como "asegurados" (críticos) para que fueran excluidos automáticamente de las propuestas de rotación. \\
\hline
\textbf{Observaciones:} & La lista de asegurados se obtenía de PSFV (ap\_Aseguramientos) y podía ser modificada por el administrador. \\
\hline
\end{tabular}
\end{table}
...
\textbf{Programador asignado:} & Brayan Castellano Cruz \\
\hline
\textbf{Descripción:} & El sistema debía generar una propuesta de rotación de circuitos (orden de afectación) basada en el déficit calculado, priorizando por bloques y excluyendo los circuitos asegurados. \\
\hline
\textbf{Observaciones:} & El algoritmo consideraba las tablas ap\_RotacionBloques y ap\_OrdenesDNC. \\
\hline
\end{tabular}
\end{table}
...
\textbf{Responsabilidades} & 
\begin{minipage}[t]{9cm}
- Almacenar código, nombre y bloque al que pertenece \\
- Conocer la carga actual (MW) desde SIGERE (ap\_curvas) \\
- Conocer el estado operativo (activo/avería/asegurado) \\
- Saber si está disponible para rotación
\end{minipage} \\
\hline
\textbf{Colaboraciones} & Bloque, Aseguramiento, OrdenRotacion \\
\hline
\end{tabular}
\end{table}
...
- Conocer los circuitos que lo componen (ap\_RotacionBloques) \\
- Establecer orden de prioridad para rotación
\end{minipage} \\
\hline
...
\textbf{Responsabilidades} & 
\begin{minipage}[t]{9cm}
- Calcular el déficit (MW\_Afectados - MW\_Asegurados) \\
- Generar propuesta de rotación priorizando por bloques \\
- Excluir circuitos asegurados de la propuesta \\
- Permitir ajuste manual por el operador \\
- Registrar la orden aprobada para historial
\end{minipage} \\
\hline
\textbf{Colaboraciones} & Circuito, Aseguramiento, Bloque, Usuario, Auditoria \\
\hline
\end{tabular}
\end{table}
...
\hline
HU4 & Visualización de circuitos & Mostrar lista de circuitos con carga, estado (activo/avería/asegurado); filtros; actualización manual (6) \\
\hline
HU5 & Gestión de circuitos asegurados & Marcar/desmarcar circuitos críticos; sincronización con PSFV; exclusión automática de propuestas (5) \\
\hline
HU6 & Cálculo automático del déficit & Suma de MW afectados, suma de MW asegurados, cálculo de diferencia (3) \\
\hline
HU7 & Generación de propuesta de rotación & Ordenar por bloques, excluir asegurados, aplicar prioridades DNC, generar lista de circuitos a rotar (8) \\
\hline
HU8 & Ajuste manual de propuestas & Agregar/quitar circuitos manualmente, registrar cambios, auditoría (4) \\
\hline
HU9 & Dashboard de monitoreo & Mostrar déficit actual, MW afectados vs asegurados, alertas visuales (3) \\
\hline
