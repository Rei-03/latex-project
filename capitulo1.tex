...
\begin{itemize}
    ...
    \item \textbf{Diseño simple y tarjetas CRC:} Se emplearon tarjetas CRC (Clase-Responsabilidad-Colaborador) durante las primeras iteraciones, identificando clases principales del dominio (\texttt{Circuito}, \texttt{Aseguramiento}, \texttt{OrdenRotacion}). Se definió una metáfora del sistema: una “fábrica de órdenes de rotación”, donde el frontend (Next.js) actúa como interfaz de supervisión y el backend (NestJS) como motor de procesamiento de reglas de negocio.
...
La elección de XP se justificó por su alineación con la arquitectura desacoplada adoptada (frontend/backend), facilitando la integración continua y la refactorización. Cada capa podía probarse independientemente, acelerando el desarrollo y reduciendo riesgos.
...
En Cuba, las decisiones de balance generación-demanda se rigen por el Despacho Nacional de Carga (DNC)~\citep{union2023}. La dependencia actual de procesos manuales y sistemas legacy como SIGERE y PSFV (basados en SQL Server 2008)~\citep{cujae2021} justifica la implementación de una capa de servicios (backend) que automatice la extracción y procesamiento de estos datos. Los protocolos del Ministerio de Energía y Minas~\citep{ministerio2023} exigen rapidez y precisión; elementos que una Single Page Application (SPA) puede potenciar al ofrecer cálculos instantáneos y visualizaciones dinámicas.
...
\begin{itemize}
    \item \textbf{Integración con sistemas legacy:} Se creó una capa de abstracción en NestJS para encapsular consultas a SQL Server 2008, ofreciendo una API REST uniforme al frontend. Esto unificó tablas críticas como \texttt{MSW\_Aseguramientos} y \texttt{ap\_curvas}.
    \item \textbf{Procesamiento en tiempo real:} Los algoritmos de priorización se ejecutan en el servidor con alta eficiencia; los resultados se transmiten al cliente en formato JSON. La comunicación asíncrona (Axios) garantiza propuestas de rotación en menos de 3 segundos.
    \item \textbf{Visualización dinámica y usabilidad:} La interfaz con React y Tailwind CSS ofrece tablas interactivas, filtros y alertas visuales, permitiendo al operador supervisar y ajustar manualmente las propuestas.
    \item \textbf{Trazabilidad y auditoría:} Se implementó un módulo de auditoría que registra cada acción (fecha, hora, MW afectados, circuitos seleccionados, y en ajustes manuales, el operador).
    \item \textbf{Escalabilidad y mantenibilidad:} La separación en capas y la refactorización continua (facilitadas por XP) aseguran que el sistema pueda evolucionar sin comprometer la estabilidad.
\end{itemize}
...
\hline
Cargas horarias (MW) & SIGERE: ap\_curvas & Consumo de megavatios por hora para circuito \\
\hline
MWh totales por circuito & SIGERE: ap\_mwxHora & Energía acumulada en megavatios-horas \\
\hline
Órdenes DNC & SIGERE: ap\_OrdenesDNC & Restricciones y prioridades establecidas por el DNC \\
\hline
Circuitos asegurados & PSFV: ap\_Aseguramientos & Circuitos críticos (hospitales, servicios esenciales) \\
\hline
Estado operativo & SIGERE: MSM\_Aseguramientos & Circuitos averiados o en mantenimiento programado \\
\hline
Bloques operativos & SIGERE: ap\_RotacionBloques & Segmentación de circuitos para la rotación \\
\hline
\end{tabular}
\caption{Variables consideradas en el proceso manual de rotación de circuitos}
...
La solución implementada con \textbf{Next.js} y \textbf{React} automatiza el cálculo del déficit y proporciona una interfaz gráfica reactiva, transformando el rol del especialista de ejecutor manual a supervisor de propuestas generadas automáticamente por el backend en \textbf{NestJS}.
...
\begin{enumerate}
    ...
    \item El análisis de la automatización en sistemas de gestión energética evidenció que las tendencias actuales apuntan hacia la virtualización mediante aplicaciones web reactivas, reduciendo tiempos de respuesta y minimizando la intervención manual. En el contexto nacional, la automatización se justificó por la necesidad de integrar datos de sistemas legacy (SIGERE y PSFV) y cumplir con las exigencias de rapidez y precisión del Ministerio de Energía y Minas.
    
    \item El diagnóstico del proceso manual reveló limitaciones críticas: tiempos de respuesta de 30–45 minutos, errores humanos en el 5–10\% de los cálculos, alta inconsistencia entre operadores, nula capacidad de simulación y acceso a datos dispersos. Estas deficiencias confirmaron la pertinencia de la plataforma web automatizada.
    
    \item La integración de los tres ejes abordados —fundamentos teórico-metodológicos, automatización energética y diagnóstico empírico— proporciona una base sólida para el diseño e implementación de la solución propuesta, estableciendo coherencia entre el problema científico y las decisiones arquitectónicas.
\end{enumerate}